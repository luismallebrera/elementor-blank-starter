<?php
/**
 * Import GAL/GDR and Municipios from CSV
...
function elementor_blank_import_galgdr_municipios() {
    // Path to CSV file
    ...
    while (($data = fgetcsv($handle, 0, ';')) !== FALSE) {
        ...
        // Check if GAL/GDR already exists
        ...
        if (!$existing_galgdr) {
            ...
            if (is_wp_error($galgdr_post_id)) {
                $results['errors'][] = 'Error creating GAL/GDR: ' . $galgdr_title;
                continue;
            }
...
add_action('admin_menu', 'elementor_blank_import_menu');
function elementor_blank_import_menu() {
    add_management_page(
        'Import GAL/GDR & Municipios',
        'Import GAL/GDR',
        'manage_options',
        'import-galgdr-municipios',
        'elementor_blank_import_page'
    );
}

function elementor_blank_import_page() {
    ?>
    <div class="wrap">
        <h1>Import GAL/GDR & Municipios</h1>
        
        <?php
        if (isset($_POST['run_import']) && check_admin_referer('import_galgdr_nonce')) {
            $results = elementor_blank_import_galgdr_municipios();
            ...
            if (is_wp_error($results)) {
                ...
                echo '<ul>';
                echo '<li>GAL/GDR created: ' . $results['galgdr_created'] . '</li>';
        ...
        <form method="post">
            ...
            <p>This will import all GAL/GDR and Municipios from the CSV file.</p>
            <p><strong>Warning:</strong> Make sure to enable the "Enable GAL/GDR & Municipios" option in the Customizer first!</p>
            <p>
